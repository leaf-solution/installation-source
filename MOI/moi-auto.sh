...
# Logo
sleep 1 && curl -s https://github.com/leaf-solution/installation-source/blob/8f6c24d5dcb41bceda033568aecd0f1703e6b1a1/logo/logo.sh | bash && sleep 1
...
if [ "$version" != "2" ];then
	sudo docker run -p 1600:1600 -p 6000:6000/tcp -p 6000:6000/udp --rm -it -w /data -v $(pwd):/data sarvalabs/moipod:latest register --data-dir $moi_dirpath --mnemonic-keystore-path $moi_dirpath/keystore.json --mnemonic-keystore-password $moi_passwd --watchdog-url https://babylon-watchdog.moi.technology/add --node-password $moi_passwd --network-rpc-url https://voyage-rpc.moi.technology/babylon --wallet-address $moi_address --node-index $moi_index --local-rpc-url http://$moi_ip:1600
        ...
        sudo docker run -p 1600:1600 -p 6000:6000/tcp -p 6000:6000/udp --rm -it -w /data -v $(pwd):/data sarvalabs/moipod:v0.7.2-port register --data-dir $moi_dirpath --mnemonic-keystore-path $moi_dirpath/keystore.json --mnemonic-keystore-password $moi_passwd --watchdog-url https://babylon-watchdog.moi.technology/add --node-password $moi_passwd --network-rpc-url https://voyage-rpc.moi.technology/babylon --wallet-address $moi_address --node-index $moi_index --local-rpc-url http://$moi_ip:1600
...
if [ "$version" != "2" ];then
    ...
    docker stop $container_name_moi
    sudo docker run --name $container_name_moi -p 1600:1600 -p 6000:6000/tcp -p 6000:6000/udp -it -d -w /data -v $(pwd):/data sarvalabs/moipod:v0.7.2-port server --babylon --data-dir $moi_dirpath --log-level DEBUG --node-password $moi_passwd --clean-db=true
    docker update --restart=unless-stopped $container_name_moi
fi
